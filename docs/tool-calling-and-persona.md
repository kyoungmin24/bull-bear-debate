...
## 3. 유저 개인화 — 설문 기반 (독자 수준 + 투자기간)

> 개인화는 두 축이다. **(톤)** 독자 수준에 맞춰 *어떻게 설명할지* — 프로파일러·tier가 담당. **(내용 강조)** 희망 투자기간에 맞춰 *어떤 근거를 부각할지* — `_horizon_directive`가 담당.
> 두 축 모두 **최종 투자판단·수치·사실은 바꾸지 않는다.**
...
main.py → orchestrator.run(survey=...)
   │
   ├─ ProfilerAgent.profile(survey)  ── LLM 1회 ──▶ "독자 맞춤 작성 지침" 문자열 (톤)
   ├─ resolve_persona_tier(survey)   ── 규칙 ──────▶ tier (beginner/intermediate/expert)
   ├─ survey["horizon"]              ── 그대로 ────▶ 단기/중기/장기 (근거 강조)
   ▼
프로필 문자열 → _persona_block 으로 Bull/Bear/Moderator 프롬프트에 주입 (톤)
tier            → few-shot 변형(쉬운/dense) 선택
horizon         → _horizon_directive 로 argue/rebut/conclude에 주입 (내용 강조)
   ▼
...
`frontend/.../InputScreen.tsx`에서 수집, `{gender, age, experience, level, terminology, depth, horizon}` 형태로 전송.
...
### 3.7 희망 투자기간 — 근거 강조 (내용 개인화)

투자기간은 *톤*이 아니라 *무엇을 봐야 하는지*를 바꾸므로, 톤 프로파일러와 **분리**해 별도 지시로 주입한다.

- `HORIZON_EMPHASIS` (`prompts.py`): 투자기간 → 부각할 근거 매핑.
  - `단기`: 촉매(이벤트)·실적 모멘텀/서프라이즈·수급·투자심리
  - `중기`: 실적 추세·밸류에이션 정상화·업황 사이클
  - `장기`: 구조적 경쟁우위(해자)·산업 성장성·재무 건전성·장기 밸류에이션
- `_horizon_directive(horizon)`: argue/rebut/conclude 프롬프트에 "이 근거를 우선 부각하되, **최종 투자판단·수치·사실은 투자기간과 무관하게 동일하게 유지**" 지시를 주입.
- **프로파일러 분리**: `build_profile_prompt`는 `horizon`을 입력에서 제외한다(톤 프로파일러가 투자기간을 톤 규칙으로 오용하지 않도록).
- horizon은 `orchestrator.run`(설문에서 추출) → `_run_round`/`_run_dynamic_rounds` → `analyst.run_action` → `build_*_prompt`로 `persona_tier`와 동일 경로로 전달된다.
- **검증**: 같은 데이터에서 단기/장기 전환 시 강조점이 이동(단기→진입 타이밍·심리 / 장기→경쟁력·성장 지속)하면서 verdict·수치는 동일함을 확인. R1(기사 단독)에서는 차이가 은근하고, 정량/다라운드에서 더 뚜렷해진다.
...
| `agents/analyst.py` | `research()`(upfront), `_run_llm`(모드/`can_top_up` 분기, 사용 프롬프트 반환), `_reflect`(숫자검증+research 근거 검토), persona_tier·horizon 전달 |
| `agents/orchestrator.py` | `_run_upfront_research`, `_merge_sides`, 프로파일러·persona_tier·horizon 배선, 면책고지 주입, base-RAG `min_score` 배선 |
| `agents/verifier.py` | **신규** 결정적 숫자검증기 (`verify_numbers`) |
| `agents/profiler.py` | **신규** `ProfilerAgent` |
| `agents/prompts.py` | 모드/액션별 `_tools_block`, `build_research_prompt`, `_persona_block`(지시형·키겸용), `_persona_directive`, `build_profile_prompt`(horizon 제외), tier·쉬운 few-shot, `resolve_persona_tier`, `HORIZON_EMPHASIS`·`_horizon_directive`, `build_reflection_prompt`(4기준+`_num_check_block`) |
...
| `frontend/.../InputScreen.tsx` | 7문항 설문 UI (희망 투자기간 포함) |
